...
// Tipos básicos
export type BudgetType = "filme" | "audio" | "imagem" | "cc" | string;
...
export default function BudgetEdit() {
  ...
  // abort entre trocas de rota / refetches
    ...
    async (silent = false) => {
      ...
      try {
        const { data: row, error } = await supabase
          .from("versions")
          .select(`id, versao, payload, budgets!inner(id, display_id, type, status)`)
          .eq("budget_id", id)
  ...
  // 🔧 Removido o redirecionamento para "/orcamentos/novo/filme".
  // O formulário é renderizado abaixo já reidratado com o payload da última versão.

  // Tecla de atalho para salvar via BudgetForm (o formulário deve escutar esse evento ou expor um método via ref)
  useEffect(() => {
  ...
  const handleSaveClick = () => {
    if (data?.id) {
      ...
      toast({ title: "Solicitado", description: "Salvando alterações…" });
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        <div className="container mx-auto px-6 py-8">
          <EmptyState
            ...
            action={{ label: "Voltar para Início", onClick: () => navigate("/") }}
            secondaryAction={{ label: "Tentar novamente", onClick: () => fetchBudget(false) }}
          />
        </div>
      </div>
    );
    ...
    <BudgetProvider>
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        <div className="container mx-auto px-6 py-8">
          ...
          {/* Formulário */}
          <motion.div
            key={data.version_id} // garante re-render ao trocar versão e reidrata o BudgetForm
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.06 }}
          >
            <BudgetForm
              // DICA: no BudgetForm, use useEffect para "recarregar" o estado quando initialPayload mudar
              budgetId={data.id}
