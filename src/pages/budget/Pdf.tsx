...
// Import de asset robusto para Vite/CDN
...
export default function BudgetPdf() {
  ...
  // Converte strings "1.234,56" para number
  const toNum = (v: any) => {
  ...
  // Preço mínimo do fornecedor (base vs opções)
  ...
  // escolhido = selecionado > mais barato (considerando opções)
  ...
  const campanhas: any[] =
    payload.campanhas || (payload.categorias ? [{ nome: "Campanha Única", categorias: payload.categorias }] : []);

  // Memo SEMPRE chamado (mesma ordem de hooks em todos os renders)
  const totaisPorCampanha = useMemo(
    () => campanhas.map((camp: any) => ({ nome: camp.nome, total: calcularTotalCampanha(camp) })),
    // stringificar para depender do conteúdo
